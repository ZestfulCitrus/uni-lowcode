<!--
 * @Author: your name
 * @Date: 2021-07-27 11:45:43
 * @LastEditTime: 2021-07-28 10:11:16
 * @LastEditors: your name
 * @Description: In User Settings Edit
 * @FilePath: \rtvue-form\src\pages\sidebar\option-bar\option-r-form-numberbox-bar.vue
-->
<template>
  <view style="margin-top: 12px; margin-left: 6px; border: 1px dotted #000">
    <view class="comInfo">
      <text class="comptype">组件类型：计数器</text>
    </view>
    <view class="comInfo">
    ...
    <view class="comInfo">
      <u-field v-model="number" label="默认数值"></u-field>
    </view>
  </view>
</template>
...
<script>
export default {
  name: 'option-r-form-numberbox-bar',
  props: {
    option: {
      type: Object,
      require: true,
  ...
  data() {
    return {
      btnChecked: false,
      number: this.option.option.value,
      options: {
  ...
  watch: {
    ...
    number() {
      this.option.option.value = parseInt(this.number)
    },
  },
  methods: {
    onMounted(editor) {
